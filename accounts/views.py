...
def register(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Gửi Email active tài khoản khi đăng ký
...
def activate(request, uidb64, token):  # Activate token tài khoản đăng ký
    try:
...
@login_required(login_url="login")
def dashboard_users(request, id=None):
    author = Account.objects.get(id=id, is_admin=True)
    context = {
        'author': author,
    }
